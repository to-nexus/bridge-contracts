...
contract BridgeStandardTest is Test, BridgeTest {
    ...
    function test_permit_deposit() public {
        ...
        vm.prank(VALIDATOR1);
        assertTrue(BridgeEthereum(address(bridgeEthereum.b)).bridgeCross(USER, amount, permitArgs, NULLDATA));

        finalize(bridgeEthereum.otherBridge, bridgeEthereum.nextIndex, address(xcross), amount, 5);
    ...
    function test_permit_depositToken() public {
        ...
        IBridgeStandard.PermitArguments memory permitArgs;
        {
            ...
            permitArgs = IBridgeStandard.PermitArguments(
                IERC20Permit(address(testTokenAtEthereum)), USER, amount, type(uint).max, v, r, s
            );
        }

        vm.prank(VALIDATOR1);
        bool ok = BridgeEthereum(address(bridgeEthereum.b)).permitBridge(
            testTokenAtEthereum, USER, amount, 0, 0, permitArgs, NULLDATA
        );
        assertTrue(ok);
