...
/**
 * @title CrossBridgeV2Deploy
 * @notice CrossBridgeV2 + ForwardLib 배포 스크립트
 * @dev `Implementation.s.sol`을 건드리지 않고 CrossBridgeV2 전용 배포 경로만 제공한다.
 *      프록시는 이미 배포되어 있다고 가정한다 — 이 스크립트는 구현 배포와 업그레이드만
 *      담당하며, 프록시 생성이나 초기화는 하지 않는다(V2는 initializer가 봉인되어 있다).
 *
 * ============================================================
 * 왜 2단계인가 — ForwardLib은 external 라이브러리다
 * ============================================================
 * `CrossBridgeV2`는 `ForwardLib`의 external 함수를 DELEGATECALL로 호출하므로,
 * 컴파일 산출물에 relocation 3곳이 남는다:
 *
 *   out/CrossBridgeV2.sol/CrossBridgeV2.json
 ...
 *
 *   forge script script/CrossBridgeV2Deploy.s.sol:CrossBridgeV2Deploy \
 *     --sig "deployForwardLib()" \
 *     --rpc-url <RPC_URL> --broadcast
 *
 * 2) 그 주소로 링크해서 CrossBridgeV2 구현 배포
 *
 *   forge script script/CrossBridgeV2Deploy.s.sol:CrossBridgeV2Deploy \
 *     --sig "deployCrossBridgeV2Impl()" \
 ...
 *
 * 4) 프록시 업그레이드 (V2는 initializer가 봉인되어 있어 초기화 호출 없음)
 *
 *   forge script script/CrossBridgeV2Deploy.s.sol:CrossBridgeV2Deploy \
 *     --sig "upgradeToCrossBridgeV2(address,address)" <PROXY> <IMPL_ADDR> \
 *     --rpc-url <RPC_URL> --broadcast
 *
 ...
 * ============================================================
 * CrossBridgeV2 구현은 런타임 22,036 B → 코드 예치만 약 4.41M 가스이며, calldata와
 ...
 */
contract CrossBridgeV2Deploy is Script {
    ...
    function deployForwardLib() public returns (address lib) {
        ...
        console.log("");
        console.log("Next: deployCrossBridgeV2Impl() with");
        console.log("  --libraries src/lib/ForwardLib.sol:ForwardLib:%s", vm.toString(lib));
    }

    /**
     * @notice CrossBridgeV2 Implementation 배포
     * @dev ForwardLib이 링크된 상태로 컴파일되어 있어야 한다(위 사용법 2 참조).
     *      프록시는 배포하지 않는다 — V2는 기존 프록시 업그레이드 전용이다.
     * @return impl 배포된 구현 주소
     */
    function deployCrossBridgeV2Impl() public returns (address impl) {
        vm.broadcast();
        CrossBridgeV2 implementation = new CrossBridgeV2();
        impl = address(implementation);

        console.log("CrossBridgeV2 Implementation deployed to:", impl);
    ...
    /**
     * @notice ForwardLib + CrossBridgeV2 구현을 한 번에 배포
    ...
    function deployAll() public returns (address lib, address impl) {
        ...
        impl = deployCrossBridgeV2Impl();
    }

    /**
     * @notice 프록시를 CrossBridgeV2 구현으로 업그레이드 (초기화 호출 없음)
     * @dev CrossBridgeV2는 `initialize`가 revert 스텁으로 봉인되어 있으므로
     *      업그레이드 시 초기화 calldata를 넘기지 않는다. 스토리지 레이아웃은
     *      CrossBridge와 동일하게 재선언되어 있어 마이그레이션이 필요 없다.
     * @param proxy 프록시 주소
     * @param impl 새 CrossBridgeV2 구현 주소
     */
    function upgradeToCrossBridgeV2(address proxy, address impl) public {
